document.addEventListener('DOMContentLoaded', () => {
    ...
    addUnitBtn.addEventListener('click', () => { const { periodId, unitId } = currentlyEditing; if (!periodId) return; const unitData = { title: document.getElementById('unit-title').value.trim(), startDate: document.getElementById('unit-start-date').value, endDate: document.getElementById('unit-end-date').value, situacion: document.getElementById('unit-situacion').value, producto: document.getElementById('unit-producto').value, competencias: Array.from(competenciesChecklist.querySelectorAll('input:checked')).map(cb => cb.value) }; if (!unitData.title) return alert('El título de la unidad es obligatorio.'); if (unitId) { const unitIndex = unitsData[periodId].findIndex(u => u.id === unitId); if (unitIndex > -1) unitsData[periodId][unitIndex] = { ...unitsData[periodId][unitIndex], ...unitData }; } else { unitsData[periodId].push({ id: `unit-${Date.now()}`, ...unitData }); } renderUnitsForPeriod(periodId); unitModal.classList.add('hidden'); });
    ...
    suggestJustificacionBtn.addEventListener('click', async () => { const prompt = `Como pedagogo experto del Currículo Nacional peruano, redacta una justificación concisa (máximo 120 palabras) para una planificación anual de ${areaSelect.value} para ${gradeSelect.value}. Basa tu redacción en este contexto estudiantil: "${document.getElementById('info-realidad').value || 'un contexto general urbano'}".`; const suggestion = await getAISuggestion(prompt, suggestJustificacionBtn); if (suggestion) document.getElementById('desc-justificacion').value = suggestion.trim(); });
    suggestPerfilBtn.addEventListener('click', async () => { const prompt = `Como experto en el CNEB de Perú, describe en un párrafo (máximo 100 palabras) el perfil de egreso esperado para un estudiante que culmina el ${gradeSelect.value} en el área de ${areaSelect.value}.`; const suggestion = await getAISuggestion(prompt, suggestPerfilBtn); if (suggestion) document.getElementById('desc-perfil').value = suggestion.trim(); });
    suggestUnitTitleBtn.addEventListener('click', async () => { const prompt = `Sugiere 5 títulos creativos y cortos para una unidad de aprendizaje del área de ${areaSelect.value} para ${gradeSelect.value}. Devuelve solo la lista de títulos.`; const suggestion = await getAISuggestion(prompt, suggestUnitTitleBtn); if (suggestion) document.getElementById('unit-title').value = suggestion.split('\n')[0].replace(/^\d+\.\s*/, '').trim(); });
    suggestUnitSituacionBtn.addEventListener('click', async () => { const prompt = `Para una unidad de aprendizaje de ${areaSelect.value} para ${gradeSelect.value} titulada "${document.getElementById('unit-title').value || "la unidad actual"}", redacta una situación significativa breve (máximo 150 palabras) que plantee un reto del contexto.`; const suggestion = await getAISuggestion(prompt, suggestUnitSituacionBtn); if (suggestion) document.getElementById('unit-situacion').value = suggestion.trim(); });
    suggestUnitProductoBtn.addEventListener('click', async () => { const prompt = `Basado en la siguiente situación significativa: "${document.getElementById('unit-situacion').value || 'una situación relacionada con el área de ' + areaSelect.value}", sugiere un producto o actuación integrador y tangible. Sé breve y directo.`; const suggestion = await getAISuggestion(prompt, suggestUnitProductoBtn); if (suggestion) document.getElementById('unit-producto').value = suggestion.trim(); });
    suggestEvalDiagnosticaBtn.addEventListener('click', async () => { const prompt = `Como experto en evaluación del Currículo Nacional peruano para el área de ${areaSelect.value} y grado ${gradeSelect.value}, redacta una breve descripción sobre la evaluación diagnóstica en el contexto de una planificación anual. Considera los saberes previos y necesidades de los estudiantes. (Máximo 100 palabras).`; const suggestion = await getAISuggestion(prompt, suggestEvalDiagnosticaBtn); if (suggestion) document.getElementById('eval-diagnostica').value = suggestion.trim(); });
    suggestEvalFormativaBtn.addEventListener('click', async () => { const prompt = `Como experto en evaluación del Currículo Nacional peruano para el área de ${areaSelect.value} y grado ${gradeSelect.value}, redacta una breve descripción sobre cómo se realizará la evaluación formativa a lo largo del año. Incluye técnicas e instrumentos generales. (Máximo 120 palabras).`; const suggestion = await getAISuggestion(prompt, suggestEvalFormativaBtn); if (suggestion) document.getElementById('eval-formativa').value = suggestion.trim(); });
    suggestEvalSumativaBtn.addEventListener('click', async () => { const prompt = `Como experto en evaluación del Currículo Nacional peruano para el área de ${areaSelect.value} y grado ${gradeSelect.value}, redacta una breve descripción sobre la evaluación sumativa al final de los periodos. (Máximo 100 palabras).`; const suggestion = await getAISuggestion(prompt, suggestEvalSumativaBtn); if (suggestion) document.getElementById('eval-sumativa').value = suggestion.trim(); });
    suggestRecursosDocenteBtn.addEventListener('click', async () => { const prompt = `Sugiere una lista de 5-7 recursos o materiales pedagógicos que un docente del área de ${areaSelect.value} para ${gradeSelect.value} de secundaria podría utilizar en su práctica. Sé conciso, solo la lista.`; const suggestion = await getAISuggestion(prompt, suggestRecursosDocenteBtn); if (suggestion) document.getElementById('recursos-docente').value = suggestion.trim(); });
    suggestRecursosEstudianteBtn.addEventListener('click', async () => { const prompt = `Sugiere una lista de 5-7 recursos o materiales que los estudiantes de ${gradeSelect.value} de secundaria, en el área de ${areaSelect.value}, podrían utilizar para su aprendizaje. Sé conciso, solo la lista.`; const suggestion = await getAISuggestion(prompt, suggestRecursosEstudianteBtn); if (suggestion) document.getElementById('recursos-estudiante').value = suggestion.trim(); });
    ...
    const renderPreview = (data) => {
        ...
        htmlContent += `<h2 class="text-xl font-semibold mt-6 mb-3">II. JUSTIFICACIÓN Y PERFIL DE EGRESO</h2><p class="mb-2"><strong>Justificación (Necesidades de Aprendizaje):</strong></p><p class="mb-4">${data.descJustificacion || 'El docente no describió la justificación.'}</p><p class="mb-2"><strong>Perfil de Egreso:</strong></p><p class="mb-4">${data.descPerfil || 'El docente no describió el perfil de egreso.'}</p>`;
